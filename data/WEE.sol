...
contract WEE is ERC20,Ownable{
	...
	modifier notReachTotalSupply(uint256 _value,uint256 _rate){
		assert(MAX_SUPPLY>=(totalSupply + ((_value * _rate))));
		_;
	}

	modifier notReachPublicFundingSupply(uint256 _value,uint256 _rate){
		assert(MAX_PUBLIC_FUNDING_SUPPLY>=(totalPublicFundingSupply + ((_value * _rate))));
		_;
	}

	modifier notReachCommonWithdrawSupply(uint256 _value,uint256 _rate){
		assert(COMMON_WITHDRAW_SUPPLY>=(totalCommonWithdrawSupply + ((_value * _rate))));
		_;
	}
...

	{
		...
		totalCommonWithdrawSupply=(totalCommonWithdrawSupply + _value);
	}
		...
		notReachPublicFundingSupply(msg.value,rate)
	{
		...
		require(((userPublicFundingEthCountMap[msg.sender]) + (msg.value))<=publicFundingPersonalEthLimit);

		processFunding(msg.sender,msg.value,rate);
		uint256 amount=((msg.value) * rate);
		totalPublicFundingSupply = (totalPublicFundingSupply + amount);

		userPublicFundingEthCountMap[msg.sender] = ((userPublicFundingEthCountMap[msg.sender]) + (msg.value));
	}
  	...
  	function transfer(address _to, uint256 _value) public  returns (bool)
 	{
		...
		require(((balances[msg.sender]) - _value)>=needLockBalance);
  	...
  	function transferFrom(address _from, address _to, uint256 _value) public returns (bool) 
  	{
		...
		require(((balances[_from]) - _value)>=needLockBalance);
		...
		onlyOwner
	{
        ...
        /*epochs*/lockEpochsMap[user].push(epoch(lockEndTime,lockAmount));
	}
