...
contract PeriodicReleaseLock {
    ...
    function freeze(address _target, uint256 _frozenAmount, uint256 _releaseAmount) byToken public returns (bool) {
        ...
        totalFrozen = (totalFrozen + _frozenAmount);

        FrozenStatus /*storage*/ memory frozenStatus = frozenStatuses[_target];
        ...
        frozenStatuses[_target] = frozenStatus;
    ...
    function release() public returns (bool) {
        ...
        require(/*frozenStatus*/ frozenStatuses[target].frozenAmount > 0);

        uint256 actualLastReleaseTimestamp;

        if (/*frozenStatus*/ frozenStatuses[target].lastReleaseTimestamp == 0) {
            actualLastReleaseTimestamp = /*frozenStatus*/ frozenStatuses[target].frozenTimestamp + firstReleasePeriod;
        } else {
            actualLastReleaseTimestamp = /*frozenStatus*/ frozenStatuses[target].lastReleaseTimestamp + regularReleasePeriod;
        }

        require(now >= actualLastReleaseTimestamp);
        /*frozenStatus*/ frozenStatuses[target].lastReleaseTimestamp = actualLastReleaseTimestamp;

        uint256 actualReleaseAmount = Math.min256(/*frozenStatus*/ frozenStatuses[target].frozenAmount, /*frozenStatus*/ frozenStatuses[target].releaseAmount);

        /*token.*/SafeERC20.safeTransfer(token, target, actualReleaseAmount);

        /*frozenStatus*/ frozenStatuses[target].frozenAmount = ((/*frozenStatus*/ frozenStatuses[target].frozenAmount) - actualReleaseAmount);
        totalFrozen = (totalFrozen - actualReleaseAmount);
    ...
    */
    function missingTokensFallback() public {
        uint256 missingTokens = ((token.balanceOf(this)) - totalFrozen);
        require(missingTokens > 0);

        TokenWithOwner tokenWithOwner = TokenWithOwner(token);

        /*token*/SafeERC20.safeTransfer(token, tokenWithOwner.owner(), missingTokens);
    }
}
