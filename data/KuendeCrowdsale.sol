...
contract KuendeCrowdsale is Ownable {
  ...
  */
  ...
  function buyTokens() public payable {
    ...
    uint256 weiAmount = ((msg.value) - (refundExcess()));

    // value after refunds should be greater or equal to minimum investment
    require(weiAmount >= minInvestment);

    // calculate token amount to be sold
    // uint256 tokens = (((weiAmount * 1 ether)) / exchangeRate);

    // update storage
    weiRaised = (weiRaised + weiAmount);
    investors[investor].weiBalance = ((investors[investor].weiBalance) + weiAmount);
    // investors[investor].tokenBalance = ((investors[investor].tokenBalance) + tokens);
  ...
  */
  ...
  function refundExcess() internal returns (uint256 excess) {
    ...
    if (limited() && !withinInvestorCap(investor, weiAmount)) {
      excess = ((((investors[investor].weiBalance) + weiAmount)) - investorCap);
      weiAmount = ((msg.value) - excess);
    }
    */
    // since limited ==> withinInvestorCap(...) == true

    // calculate excess for crowdsale cap
    if (!/*withinCap(weiAmount)*/ (weiRaised + weiAmount <= cap)) {
      excess = (excess + ((((weiRaised + weiAmount)) - cap)));
    }
  ...
  function validPurchase() internal view {
    ...
    require (/*whitelisted(msg.sender)*/ investors[msg.sender].whitelisted);            // check if investor is whitelisted
    require (/*withinCap(0)*/ weiRaised <= cap);                       // check if purchase is within cap

    //require (withinInvestorCap(msg.sender, 0));   // check if purchase is within investor cap
    require (investors[msg.sender].weiBalance <= investorCap);
  }
  ...
  function withinCap(uint256 weiAmount) internal view returns (bool) {
    return (weiRaised + weiAmount) <= cap;
  }
  ...
  function withinInvestorCap(address investor, uint256 weiAmount) internal view returns (bool) {
      // return limited() ? ((investors[investor].weiBalance) + weiAmount) <= investorCap : true;
      return
          !(startTime <= now && now < (startTime + 3 days)) ||
          (((investors[investor].weiBalance) + weiAmount) <= investorCap);
  }
  ...
  function limited() internal view returns (bool) {
    return  startTime <= now && now < (startTime + 3 days);
  }
}
