...
contract Layerx is IERC20, Owned {
    ...
    */    
    function lock(uint payment) external paused {
        ...
        holder.layerLocked = ((holder.layerLocked) + payment);
        stakeHolders[msg.sender] = holder;
        stake.layerLockedTotal = ((stake.layerLockedTotal) + payment);
    ...
    */    
    function unlock() external paused {
        ...
        Stake memory stake = stakes[stakeNum];
        stake.layerLockedTotal = ((stake.layerLockedTotal) - (holder.layerLocked));
        stakes[stakeNum] = stake;
    ...
    */      
    function closeStake() external {
        ...
        Stake memory stake = stakes[stakeNum]; 
        require(now >= ((stake.start) + stakePeriod), 'You cannot call this function until stakePeriod is over');
        
        stake.end = now;
        stake.ethReward = ((stake.ethReward) + totalEthRewards);
  
        uint amtLayerx = ((((((stake.end) - (stake.start))) * amtByDay)) / 1 days);
    ...
    */
    function withdraw(uint index) external paused exist(index) {
        ...
        if(rwds.layerLocked > 0) {
            rwds.layersx = ((((rwds.layerLocked) * (stake.layerxReward))) / (stake.layerLockedTotal));
            rwds.eth = ((((rwds.layerLocked) * (stake.ethReward))) / (stake.layerLockedTotal));            
        }
        rewards[msg.sender][index] = rwds;
        emit logWithdraw(msg.sender, rwds.layersx, rwds.eth, index, now);
            
        if(rwds.layersx > 0) {
            balances[owner] = ((balances[owner]) - (rwds.layersx));
            balances[msg.sender] = ((balances[msg.sender]) + (rwds.layersx));  
