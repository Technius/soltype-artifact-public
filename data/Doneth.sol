...
contract Doneth {
    ...
    function init() public {
        ...
        members[tx.origin].shares = ((members[tx.origin].shares) + 1);
    ...
    function changeSharedExpenseAllocation(uint256 newAllocation) public onlyOwner() {
        ...
        if ((newAllocation - sharedExpenseWithdrawn) > this.balance) revert();
    ...
    function allocateShares(address who, uint256 amount) public onlyAdmin() onlyExisting(who) {
        ...
        if (amount > currentShares) {
            addShare(who, (amount - currentShares));
        } else {
            removeShare(who, (currentShares - amount));
        }
    }
    ...
    function addShare(address who, uint256 amount) public onlyAdmin() onlyExisting(who) {
        ...
        members[who].shares = ((members[who].shares) + amount);
    ...
    function withdraw(uint256 amount) public onlyExisting(msg.sender) {
        ...
        if (amount > (newTotal - (members[msg.sender].withdrawn))) revert();
        
        members[msg.sender].withdrawn = ((members[msg.sender].withdrawn) + amount);
    ...
    function withdrawToken(uint256 amount, address token) public onlyExisting(msg.sender) {
        ...
        if (amount > (newTotal - (members[msg.sender].tokensWithdrawn[token]))) revert();

        members[msg.sender].tokensWithdrawn[token] = ((members[msg.sender].tokensWithdrawn[token]) + amount);
        tokens[token] = ((tokens[token]) + amount);
    ...
    function withdrawSharedExpense(uint256 amount, address to) public onlyAdmin() {
        ...
        sharedExpenseWithdrawn = (sharedExpenseWithdrawn + amount);
        to.transfer(amount);
    ...
    function calculateTotalWithdrawableAmount(address who) public constant onlyExisting(who) returns (uint256) {
        ...
        uint256 balanceSum = ((this.balance) + totalWithdrawn);
        balanceSum = (balanceSum - sharedExpense);
        balanceSum = (balanceSum + sharedExpenseWithdrawn);
        ...
        uint256 ethPPN = (ethPerSharePPN * (members[who].shares));
        uint256 ethVal = (ethPPN / (10**PRECISION)); 
        return ethVal;
    }


    function calculateTotalWithdrawableTokenAmount(address who, address token) public constant returns(uint256) {
        uint256 balanceSum = ((checkERC20Balance(token)) + (tokens[token]));

        // Need to use parts-per notation to compute percentages for lack of floating point division
        uint256 tokPerSharePPN = balanceSum.percent(totalShares, PRECISION); 
        uint256 tokPPN = (tokPerSharePPN * (members[who].shares));
        uint256 tokVal = (tokPPN / (10**PRECISION)); 
        return tokVal;
    }
