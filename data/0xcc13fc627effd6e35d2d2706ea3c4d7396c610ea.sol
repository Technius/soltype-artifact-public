...
contract IDXM is Owned, SafeMath {
  ...
  function uploadBalances(address[] addresses, uint256[] balances) onlyOwner {
    ...
    for (uint256 i = 0; i < uint256(addresses.length); i++) {
      require(balances[i] >= balanceOf[addresses[i]]);
      require(sum + (balances[i] - balanceOf[addresses[i]]) >= sum);
      sum = sum + balances[i] - balanceOf[addresses[i]];
      balanceOf[addresses[i]] = balances[i];
    }
    require(balanceOf[owner] >= sum);
    balanceOf[owner] = balanceOf[owner] - sum;
  }
  ...
  function precalculate() internal returns (bool success) {
    baseFeeDivisor = 10**8; // pow10(1, feeDecimals);
    feeDivisor = 10**16; // pow10(1, feeDecimals + decimals);
    singleIDXMQty = 10**8; //pow10(1, decimals);
  }
