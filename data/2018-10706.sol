...
contract Token is TokenI {
    ...
    address _vaultAddress
        ...
        totalSupply = initialSupply * 10 ** uint256(decimalUnits); // EVAL: unsafe, unsafe
    ...
    function transfer(address _to, uint256 _value) realUser(_to) moreThanZero(_value) public returns (bool) {
        ...
        balanceOf[msg.sender] = balanceOf[msg.sender] - _value;                     // Subtract from the sender // EVAL: safe
        balanceOf[_to] = balanceOf[_to] + _value;                            // Add the same to the recipient // EVAL: safe
    ...
    function transferFrom(address _from, address _to, uint256 _value) realUser(_from) realUser(_to) moreThanZero(_value) public returns (bool success) {
        ...
        balanceOf[_from] = balanceOf[_from] - _value;                           // Subtract from the sender // EVAL: safe
        balanceOf[_to] = balanceOf[_to] + _value;                             // Add the same to the recipient // EVAL: safe
        allowance[_from][msg.sender] = allowance[_from][msg.sender] + _value; // EVAL: unsafe
    ...
    function transferMulti(address[] _to, uint256[] _value) public returns (uint256 amount){
        ...
        for(uint8 j; j<len; j++){ // EVAL: safe
            amount += _value[j]*10**uint256(decimals); // EVAL: unsafe, unsafe
        }
        require(balanceOf[msg.sender] >= amount);
        for(uint8 i; i<len; i++){ // EVAL: safe
            address _toI = _to[i];
            uint256 _valueI = _value[i]*10**uint256(decimals); // EVAL: unsafe
            balanceOf[_toI] += _valueI; // EVAL: unsafe
            balanceOf[msg.sender] -= _valueI; // EVAL: safe
    ...
    function freeze(address _user, uint256 _value, uint8 _step) moreThanZero(_value) onlyController public returns (bool success) {
        _value=_value*10**uint256(decimals); // EVAL: unsafe
    ...
    function _freeze(address _user, uint256 _value, uint8 _step) moreThanZero(_value) private returns (bool success) {
        ...
        balanceOf[_user] = balanceOf[_user] - _value; // EVAL: safe
        freezeOf[_step][lastFreezeSeq[_step]] = FreezeInfo({user:_user, amount:_value});
        lastFreezeSeq[_step]++; // EVAL: safe
    ...
    function unFreeze(uint8 _step) onlyOwner public returns (bool unlockOver) {
        ...
        unlockOver=false;
        //uint8  _start=0;
        for(uint8 _start=0; _end>_start; _end--){ // EVAL: safe
            FreezeInfo storage fInfo = freezeOf[_step][_end-1]; // EVAL: safe
            uint256 _amount = fInfo.amount;
            balanceOf[fInfo.user] += _amount; // EVAL: unsafe
            delete freezeOf[_step][_end-1]; // EVAL: safe
            lastFreezeSeq[_step]--; // EVAL: safe
    ...
    function generateTokens(address _user, uint _amount) onlyController public returns (bool) {
        _amount=_amount*10**uint256(decimals); // EVAL: unsafe
        return _generateTokens(_user,_amount);
    }

    function _generateTokens(address _user, uint _amount)  private returns (bool) {
        require(balanceOf[owner] >= _amount);
        balanceOf[_user] += _amount; // EVAL: unsafe
        balanceOf[owner] -= _amount; // EVAL: safe
    ...
    function _destroyTokens(address _user, uint256 _amount) private returns (bool) {
        ...
        balanceOf[owner] += _amount; // EVAL: unsafe
        balanceOf[_user] -= _amount; // EVAL: safe
    ...
    function changeOwner(address newOwner) onlyOwner public returns (bool) {
        balanceOf[newOwner] += balanceOf[owner]; // EVAL: unsafe
        balanceOf[owner] = 0;
        owner = newOwner;
        return true;
    }
    ...
    function changeAirdropTotalQty(uint256 _airdropTotalQty) onlyController public {
        uint256 _token =_airdropTotalQty*10**uint256(decimals); // EVAL: unsafe
    ...
    function() payable public {
        ...
        if(msg.value==0){//空投
            ...
            tokenValue=airdropQty*10**uint256(decimals); // EVAL: unsafe
            airdropOf[_user]=tokenValue;
            airdropTotalQty-=airdropQty; // EVAL: safe
            ...
            totalCollected +=msg.value; // EVAL: unsafe
            require(vaultAddress.send(msg.value));//Send the ether to the vault
            tokenValue = (msg.value/1 ether)*(tokensPerEther*10 ** uint256(decimals)); // EVAL: unsafe, unsafe
            require(_generateTokens(_user, tokenValue));
